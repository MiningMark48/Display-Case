...
import com.miningmark48.displaycase.tile.TileEntityFishingRodHolder;
...
public class BlockFishingRodHolder extends BlockDisplayCaseBase {
    ...
    private static AxisAlignedBB BOUNDING_BOX_NORTH = new AxisAlignedBB(0.0D, 0.8125D, 0.890625D, 1D, 1D, 1D);
    private static AxisAlignedBB BOUNDING_BOX_EAST = new AxisAlignedBB(0.0D, 0.8125D, 0.0D, 0.109375D, 1D, 1D);
    private static AxisAlignedBB BOUNDING_BOX_SOUTH = new AxisAlignedBB(0.0D, 0.8125D, 0.0D, 1D, 1D, 0.109375D);
    private static AxisAlignedBB BOUNDING_BOX_WEST = new AxisAlignedBB(0.890625D, 0.8125D, 0.0D, 1D, 1D, 1D);

    public static final PropertyDirection FACING = PropertyDirection.create("facing");

    public BlockFishingRodHolder() {
        super();
    ...
    public TileEntity createNewTileEntity(World worldIn, int meta) {
        return new TileEntityFishingRodHolder();
    }

    //Custom Tooltip
    @Override
    public void addInformation(ItemStack par1ItemStack, @Nullable World world, List par3List, ITooltipFlag par4) {
        par3List.add(TextFormatting.LIGHT_PURPLE + ModTranslate.toLocal("tooltip.block.fishing_rod_holder.line1"));
        par3List.add(TextFormatting.YELLOW + ModTranslate.toLocal("gui.display_case.info.fishing_rod.line1"));
    }
    ...
    public boolean onBlockActivated(World world, BlockPos pos, IBlockState state, EntityPlayer player, EnumHand hand, EnumFacing side, float hitX, float hitY, float hitZ){
        if (player != null) {
            if (!player.isSneaking()) {
                    player.openGui(Reference.MOD_ID, GuiHandler.gui_id_dc_fishing_rod, world, pos.getX(), pos.getY(), pos.getZ());
                return true;
            }
        }
        return false;
